...
- **Attribution is a product requirement**, and the service never returns an image URL without the means to credit it. The credit never sits inside the photograph's own frame: a chip painted over every image is one the reader learns to ignore, and one every overlay above it has to work around. Instead each photograph is credited by the surface that owns it — a place in its details view, a trip cover on the trip's own screen, the landing photography in the page footer — so the photographer and the provider are always named and always one tap away, and a photograph shown small leans on the surface that shows it large.
