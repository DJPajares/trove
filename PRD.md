...
- **Attribution is a product requirement.** The photographer and the provider are credited wherever an editorial photograph is shown at a size that can carry the credit, and the service never returns an image URL without the means to credit it. A list thumbnail is not such a size — a credit under every row turns a list of places into a list of photographers — so a thumbnail credits its photograph through the place's details view, which is where the photograph is opened.
