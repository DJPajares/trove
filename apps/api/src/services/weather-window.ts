import { getLocalDate, shiftDateOnly } from './trip-rules.js';
...
export function resolveForecastWindow(
  ...
  const localEnd = shiftDateOnly(earliestToday, WEATHER_FORECAST_HORIZON_DAYS);
  ...
  const providerEnd = shiftDateOnly(getLocalDate(now, 'UTC'), WEATHER_FORECAST_HORIZON_DAYS);
