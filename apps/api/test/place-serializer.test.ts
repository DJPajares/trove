...
test('a provider Place carries the coordinates its snapshot resolved', () => {
  ...
  assert.deepEqual(place.location, { latitude: 1.2966, longitude: 103.8485, timeZone: null });
  assert.equal(place.snapshot?.name, 'National Museum of Singapore');
  assert.equal(place.snapshot?.address, '93 Stamford Rd, Singapore 178897');
  assert.equal(place.snapshot?.category, 'things_to_do');
  assert.equal(place.snapshot?.stale, false);
});
...
test("a provider Place's time zone stays null rather than being invented", () => {
  ...
  assert.equal(place.location?.timeZone, null);
  assert.equal(place.snapshot?.utcOffsetMinutes, 480);
});
...
test('a snapshot past its 30-day life still renders, and says it is stale', () => {
  ...
  assert.equal(place.snapshot?.name, 'National Museum of Singapore');
  assert.equal(place.snapshot?.stale, true);
});
...
test('a snapshot refreshed during this request wins over the row it was read with', () => {
  ...
  assert.equal(place.snapshot?.name, 'Refreshed name');
});
...
test('an expense or Memory keeps its Trip Place identity and gains the snapshot', () => {
  ...
  assert.equal(reference.id, 'trip-place-1');
  assert.equal(reference.placeId, 'place-1');
  assert.equal(reference.kind, 'provider');
  assert.equal(reference.name, null);
  assert.equal(reference.snapshot?.name, 'National Museum of Singapore');
});
