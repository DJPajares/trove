...
const buttonVariants = cva(
  "group/button inline-flex shrink-0 items-center justify-center rounded-[var(--radius-md)] border border-transparent bg-clip-padding text-sm font-medium whitespace-nowrap transition-[color,background-color,border-color,box-shadow,transform] duration-[var(--motion-standard)] ease-[var(--ease-standard)] outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        default:
          'bg-primary text-primary-foreground shadow-[var(--shadow-control)] hover:bg-brand-strong hover:shadow-none',
        outline:
          'border-border-strong bg-background shadow-[var(--shadow-control)] hover:bg-surface-hover hover:text-foreground hover:shadow-none aria-expanded:bg-surface-hover aria-expanded:text-foreground dark:bg-input/30',
        secondary:
          'border-border-subtle bg-secondary text-secondary-foreground hover:border-border-strong hover:bg-[color-mix(in_oklch,var(--secondary),var(--foreground)_5%)] aria-expanded:bg-secondary aria-expanded:text-secondary-foreground',
        ghost:
      ...
      size: {
        ...
        'icon-lg': 'size-11',
        'quick-action':
          "h-11 w-full flex-col gap-0.5 px-1 text-[0.7rem] leading-none [&_svg:not([class*='size-'])]:size-3.5",
      },
    },
    defaultVariants: {
      variant: 'default',
