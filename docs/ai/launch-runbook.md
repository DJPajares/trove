...
| Model change fallout | `errorCode` in `invalid_response`/`timeout` spiking right after a `TROVE_AI_MODEL` change | The new model reasons more, and reasoning is billed against `TROVE_AI_MAX_OUTPUT_TOKENS`. Lower `TROVE_AI_THINKING_BUDGET_TOKENS` or raise the output cap; a truncated answer reports finish reason `length` |
